...
} from '../core/commentRuntimeContext';
...
import { getHiddenThreadIdsForUser, type ThreadVisibilityContext } from '../core/threadVisibility';
...
export function useContentComments({
  ...
  const runContentStateSync = () => {
    ...
    try {
      syncContentCommentAnchors(editor, doc, registry, buildVisibilityContext());
    } finally {
